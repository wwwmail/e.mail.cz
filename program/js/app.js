...
function rcube_webmail()
{
  ...
  this.add2attachment_list = function(name, att, upload_id)
  {
    ...
    if (!att.complete && att.frame) {
      ...
      att.html = '<div id="loader-wrapper"><div id="loader"></div></div>';
    }
  ...
  this.ksearch_query_results = function(results, search, reqid)
  {
    ...
    if (!this.ksearch_pane) {
      ...
      this.ksearch_pane = $('<div>')
        ...
        .css({position: 'absolute', 'z-index': 30001})
        .append(ul)
        .appendTo(document.body);
    ...
    if (reqid && this.ksearch_pane.data('reqid') == reqid) {
      ...
      $(this.ksearch_input).parent('ul').css('background', 'red');
      var pos = $(this.ksearch_input).offset();
      console.log();
      var pos = $(this.ksearch_input).parent('li').parent('ul').offset();
          ...
          var contactName = text;
			...
			if(contactName){
				...
				if(contactName[1]){
					...
					$('li #'+'rcmkSearchItem'+id+' .letter-name').text(FirstLast);	
				}
				
        $('<li>').attr('id', 'rcmkSearchItem' + id)
          .attr('role', 'option')
          .html('<div class="letter-name">'+FirstLast+'</div>' + this.quote_html(text.replace(new RegExp('('+RegExp.escape(value)+')', 'ig'), '##$1%%')).replace(/##([^%]+)%%/g, '<b>$1</b>'))
          .addClass(type || '')
          .appendTo(ul)
  ...
  this.set_quota = function(content)
  {
    if (this.gui_objects.quotadisplay && content && content.type == 'text')
      ...
      $(this.gui_objects.quotadisplay).text(content.title).attr('title', content.title);

    this.triggerEvent('setquota', content);
    this.env.quota_content = content;
    
     if(content.percent == 0){
        $('pie-chart').attr('percent', '1');
    }else if(content.percent == 100){
        
        $('pie-chart').attr('percent', '100');
    }else{
        $('pie-chart').attr('percent', content.percent||0);
    }
    
    $('.text-info').text(content.title);
