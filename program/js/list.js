...
insert_row: function(row, before)
{
  ...
  if (this.checkbox_selection) {
    ...
    chbox.tabIndex = -1;
    //console.log(chbox);
    $(label).addClass('costum-checkbox-mail-list');
    //$(label).attr('for','input'+row.id );
    chbox.id = 'input'+row.id;
    ...
    label.onclick = function(e) {
      if (!$(e.target).is('input')) {
        ...
        if(chbox.checked){
           ...
           $('.header.header-menu.box-shadow').removeClass('disabl-menu-header');
           
        }else{
            
            if($('#messagelist-content table tr').hasClass('selected')){
           
            }else{
               $('.header.header-menu.box-shadow').addClass('disabl-menu-header');
            }
        }
   ...
   //  $(chbox).wrap('<label class="checkbox-x__label"></label>').before('<span class="avatar__first-name">a</span>').after('<div class="checkbox-x__body"><span class="checkbox-x__icon"></span></div>');
